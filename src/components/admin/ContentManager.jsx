...
const SECTIONS = {
  ...
  PlayerStat: { label: 'Live Player Stats', title: 'player_name', liveStats: true, readOnly: true },
...
function LiveStatRow({ row }) {
  ...
  return <div className="rounded-lg border border-white/5 bg-black/25 p-3"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-bold text-white">{row.player_name}</p><p className="text-xs text-gray-500">{row.server_name} · {row.source.toUpperCase()}{row.online ? ' · ONLINE' : ''}</p></div><div className="flex flex-wrap gap-2">{details.map((detail) => <span key={detail} className="rounded border border-emerald-400/15 bg-emerald-400/5 px-2 py-1 text-[11px] font-mono text-gray-300">{detail}</span>)}</div></div></div>;
}

export default function ContentManager() {
  const sectionKeys = Object.keys(SECTIONS); const [entity, setEntity] = useState(sectionKeys[0]); const [rows, setRows] = useState([]); const [editing, setEditing] = useState(null); const [form, setForm] = useState({}); const [loading, setLoading] = useState(false); const [message, setMessage] = useState(''); const section = SECTIONS[entity];
  ...
  const loadLiveStats = async () => {
    ...
    const sevenRows = sevenResponse.ok ? await sevenResponse.json() : []; const dayzRows = dayzResponse.ok ? await dayzResponse.json() : [];
    const sevenServer = servers.find(is7DTD); const dayzServer = servers.find(isDayZ);
    return [
      ...(sevenServer ? sevenRows.map((row) => ({ id: `7dtd-${row.playerId}`, player_name: row.name || row.aliases?.[0] || 'Unknown survivor', server_name: sevenServer.name, source: '7dtd', ...row })) : []),
      ...(dayzServer ? dayzRows.map((row) => ({ id: `dayz-${row.id}`, player_name: row.displayName || 'Unknown survivor', server_name: dayzServer.name, source: 'dayz', ...row })) : []),
  ...
  const startNew = () => { setEditing('new'); const next = emptyFor(section); if (entity === 'Server') Object.assign(next, { query_type: 'protocol-valve', query_host: '127.0.0.1', query_port: 26903, rcon_enabled: false, rcon_port: 2306, banner_position: 'center' }); setForm(next); };
  const startEdit = (row) => { setEditing(row.id); setForm(formForRow(section, row)); }; const cancel = () => { setEditing(null); setForm({}); };
  const save = async () => { setLoading(true); setMessage(''); try { const payload = formForRow(section, form); if (payload.date && !String(payload.date).endsWith('Z')) payload.date = new Date(payload.date).toISOString(); if (editing === 'new') await base44.entities[entity].create(payload); else await base44.entities[entity].update(editing, payload); setMessage(editing === 'new' ? 'Created successfully.' : 'Updated successfully.'); cancel(); await load(); } catch (error) { setMessage(error.message); } finally { setLoading(false); } };
  const remove = async (id) => { if (!window.confirm('Delete this item permanently?')) return; setLoading(true); try { await base44.entities[entity].delete(id); setMessage('Deleted successfully.'); await load(); } catch (error) { setMessage(error.message); } finally { setLoading(false); } };
  ...
  return <div className="space-y-5"><div className="flex flex-wrap gap-2">{sectionKeys.map((key) => <button key={key} onClick={() => setEntity(key)} className={`rounded-lg border px-3 py-2 text-xs font-bold ${entity === key ? 'border-emerald-400/50 bg-emerald-400/10 text-emerald-300' : 'border-white/10 text-gray-400'}`}>{SECTIONS[key].label}</button>)}</div>
    <div className="rounded-xl border border-emerald-400/15 bg-black/30 p-5"><div className="mb-5 flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-lg font-black text-white">{section.label}</h2><p className="text-xs text-gray-500">{summary}{section.liveStats ? ' · automatically populated from live game data' : ''}</p></div><div className="flex gap-2"><button onClick={load} className="rounded-lg border border-white/10 p-2 text-gray-400 hover:text-white"><RefreshCw size={16} /></button>{!section.readOnly && !section.statusOnly && <button onClick={startNew} className="flex items-center gap-2 rounded-lg border border-emerald-400/40 bg-emerald-400/10 px-3 py-2 text-xs font-bold text-emerald-300"><Plus size={15} /> NEW</button>}</div></div>
